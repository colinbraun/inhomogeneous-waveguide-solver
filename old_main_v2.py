...
def interpolate_partial_x(node1, node2, node3, phi1, phi2, phi3, x, y):
    ...
    n2 = phi2 * b2
    n3 = phi3 * b3
    return (n1 + n2 + n3) / 2 / area


def interpolate_partial_y(node1, node2, node3, phi1, phi2, phi3, x, y):
    x1, y1 = node1[0], node1[1]
    x2, y2 = node2[0], node2[1]
    x3, y3 = node3[0], node3[1]
    b1 = y2-y3
...
if __name__ == '__main__':
    ...
    #     x1, y1 = all_edges[element.edges[0]].line()
    #     x2, y2 = all_edges[element.edges[1]].line()
    #     x3, y3 = all_edges[element.edges[2]].line()
    #     xtotal = x1 + x2 + x3
    #     ytotal = y1 + y2 + y3
    #     xy = np.array([xtotal, ytotal]).transpose()
    #     xy = np.unique(xy, axis=0)
    #     print(xy)
    #     # plt.plot(xtotal, ytotal, marker='o')
    #     color = "blue" if element.permittivity == 1 else "red"
    #     plt.gca().add_patch(patches.Polygon(xy, color=color))
    #     print(n)
    #     plt.waitforbuttonpress()
    #     # plt.pause(0.5)
    count = 0
    for n, edge in enumerate(boundary_edge_numbers):
        x1, y1 = all_edges[edge].line()
        # x1, y1 = all_nodes[edge][0], all_nodes[edge][1]
    ...
    print(count)
    edges_set = set()
    for n, edge in enumerate(all_edges):
        if edge in edges_set or edge.flip() in edges_set:
            print("TRUE")
            exit()
        edges_set.add(edge)
    element_list = []
    ...
    for k0 in k0s:
        ...
        # The A-matrix on the LHS of the weak form equation that we wish to solve
        a = np.zeros([len(remap_inner_node_nums), len(remap_inner_node_nums)])
        # The B-matrix on the RHS of the weak form equation that we wish to solve
        b = np.zeros([len(remap_inner_node_nums), len(remap_inner_node_nums)])
        ...
        for n, element in enumerate(connectivity):
            ...
            for i in range(len(nodes)):
                ...
                for j in range(len(nodes)):
                    ...
                    a[remap_inner_node_nums[nodes[i]], remap_inner_node_nums[nodes[j]]] += value1
                    # b[remap_inner_node_nums[nodes[i]], remap_inner_node_nums[nodes[j]]] += l * 2 * area / 24
                    b[remap_inner_node_nums[nodes[i]], remap_inner_node_nums[nodes[j]]] += value2
    ...
    new_mat = np.linalg.inv(b) @ a
    eigen_values, eigen_vectors = np.linalg.eig(new_mat)
    # Take the transpose such that each row of the matrix now corresponds to an eigenvector (helpful for sorting)
    eigen_vectors = eigen_vectors.transpose()
    # Sort the eigenvalues and eigenvectors by the eigenvalues
    temp = sorted(zip(eigen_values, eigen_vectors), key=lambda pair: pair[0])
    eigen_values = np.array([x for x, _ in temp])
    eigen_vectors = np.array([x for _, x in temp])
    print(eigen_values)
    ...
    x_max = np.amax(all_nodes[:, 0])
    y_max = np.amax(all_nodes[:, 1])
    # Create a rectangular grid of points that is inscribed by the geometry
    num_x_points = 100
    x_points = np.linspace(x_min, x_max, num_x_points)
    num_y_points = 100
    y_points = np.linspace(y_min, y_max, num_y_points)
    # This is either Ez or Hz depending on whether we are finding the TE or TM modes. They are computed the same way.
    Ez_or_Hz = np.zeros([num_x_points, num_y_points])
    Hx = np.zeros([num_x_points, num_y_points])
    Hy = np.zeros([num_x_points, num_y_points])
    Ex = np.zeros([num_x_points, num_y_points])
    Ey = np.zeros([num_x_points, num_y_points])
    # Iterate over each point, find which triangle it is in, and evaluate it
    mode = 4
    for i in range(num_y_points):
        pt_y = y_points[i]
        for j in range(num_x_points):
            pt_x = x_points[j]
            element_num = -1
            for k, item in enumerate(connectivity):
                element = item.nodes
                if is_inside(all_nodes[element[0]], all_nodes[element[1]], all_nodes[element[2]], pt_x, pt_y):
                    element_num = k
                    break
            if element_num != -1:
                phi1 = 0 if not element[0] in remap_inner_node_nums else eigen_vectors[mode, remap_inner_node_nums[element[0]]]
                phi2 = 0 if not element[1] in remap_inner_node_nums else eigen_vectors[mode, remap_inner_node_nums[element[1]]]
                phi3 = 0 if not element[2] in remap_inner_node_nums else eigen_vectors[mode, remap_inner_node_nums[element[2]]]
                Ez_or_Hz[i, j] = item.nodal_interpolate(phi1, phi2, phi3, pt_x, pt_y)
                # Ez_or_Hz[i, j] = interpolate(all_nodes[element[0]], all_nodes[element[1]], all_nodes[element[2]], phi1, phi2, phi3, pt_x, pt_y)
                partial_x = interpolate_partial_x(all_nodes[element[0]], all_nodes[element[1]], all_nodes[element[2]], phi1, phi2, phi3, pt_x, pt_y)
                partial_y = interpolate_partial_y(all_nodes[element[0]], all_nodes[element[1]], all_nodes[element[2]], phi1, phi2, phi3, pt_x, pt_y)
                if False:
                    Hx[i, j] = -partial_x
                    Hy[i, j] = -partial_y
                    Ex[i, j] = -partial_y
                    ...
                    Hx[i, j] = partial_y
                    Hy[i, j] = -partial_x
                    # The below 2 should be negative, but having them not negative gives a better looking graph
                    Ex[i, j] = partial_x
                    Ey[i, j] = partial_y
                # print("Found triangle point lies in")
                # all_elements.add(k)
                # print(k)
            else:
                Ez_or_Hz[i, j] = 0
                # print(f"Point ({pt_x}, {pt_y}) did not lie in a triangle")
                # print("Did not lie in any triangle")

    # Uncomment the below line to show a scatter plot outline of the geometry
    # plot_edge_nodes(all_nodes, edge_node_numbers)
    # Finally, the result Ez is obtained. Plot it.
    plt.figure()
    color_image = plt.imshow(Ez_or_Hz, extent=[x_min, x_max, y_min, y_max], cmap="cividis")
    plt.colorbar(label="Ez" if False else "Hz")
    mode_type = "TM" if False else "TM"
    mode_number = "21"
    plt.title("$\mathregular{" + mode_type + "_{" + mode_number + "}}$ Mode")
    x = np.linspace(x_min, x_max, num_x_points)
    y = np.linspace(y_min, y_max, num_y_points)
    X, Y = np.meshgrid(x, y)
    # Don't plot 100x100 arrows of the vector field, they will be too small and compact, showing nothing of interest
    skip = (slice(None, None, 5), slice(None, None, 5))
    plt.quiver(X[skip], Y[skip], Ex[skip], Ey[skip], color="red", width=0.005)
    # plt.quiver(X[skip], Y[skip], Hx[skip], Hy[skip], color="blue")
